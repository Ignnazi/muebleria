...
public class cotizar implements opcion {

    private final muebleR repo;
    private final varianteS varS;

    public cotizar(muebleR repo, varianteS varS) {
        this.repo = repo;
        this.varS = varS;
    }
    ...
    public String ejecutar(List<comando> items) {
        ...
        for (comando it : items) {
            ...
            if (m.getEstado() == null || !m.getEstado().equalsIgnoreCase("activo")) {
                return "el mueble con id " + it.getIdMueble() + " está inactivo y no se puede cotizar, elige productos con estado activo!";
            }
            if (m.getStock() < it.getCantidad()) {
                return "stock insuficiente en el mueble con id " + it.getIdMueble();
            }
        }
        ...
        return "- COTIZACIÓN GENERADA - TOTAL [" + total + "] PESOS";
    }
}
